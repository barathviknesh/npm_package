...
var httpAdapter = isHttpAdapterSupported && function httpAdapter(config) {
  return new es6Promise.Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
...
var xhrAdapter = isXHRAdapterSupported && function (config) {
  return new es6Promise.Promise(function dispatchXhrRequest(resolve, reject) {
...
function dispatchRequest(config) {
  ...
  return adapter(config).then(function onAdapterResolution(response) {
    ...
    return es6Promise.Promise.reject(reason);
  });
}
...
class Axios {
  ...
  request(configOrUrl, config) {
    ...
    if (!synchronousRequestInterceptors) {
      ...
      promise = es6Promise.Promise.resolve(config);
    ...
    try {
      ...
      return es6Promise.Promise.reject(error);
    }
...
class CancelToken {
  constructor(executor) {
    ...
    this.promise = new es6Promise.Promise(function promiseExecutor(resolve) {
    ...
    this.promise.then = onfulfilled => {
      let _resolve;
      // eslint-disable-next-line func-names
      const promise = new es6Promise.Promise(resolve => {
...
axios.all = function all(promises) {
  return es6Promise.Promise.all(promises);
};
...
const razorpayCall = ({
  ...
  script.onload = async () => {
    try {
      ...
      const {
        data
      } = await axios$1.get(process.env.BASE_URL + '/api/v1/payment/get-razorpay-key');
      const options = {
        key: data.data,
        ...
        handler: async function (response) {
          const result = await axios$1.post(process.env.BASE_URL + '/api/v1/payment/pay-order', {
            amount: amount,
